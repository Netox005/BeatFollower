...
var Color = (function(window){
	...
	* });
    ...
    Color.prototype.brighter = function(factor) {
        if(!factor) factor = .7;
        var that = this.copy(this), i = Math.floor(1 / (1 - factor));
        var rgb = [this._red, this._green, this._blue];
        if(rgb[0] === 0 && rgb[1] === 0 && rgb[2] === 0) return new Color('rgb(' + i + ', ' + i + ', ' + i + ', ' + this._alpha + ')');
        ...
        return new Color('rgba(' + rgb[0] + ', ' + rgb[1] + ', ' + rgb[2] + ', ' + this._alpha + ')');
    };

    Color.prototype.darker = function(factor) {
        if(!factor) factor = .7;
        var rgb = [this._red, this._green, this._blue];
        rgb.forEach(function(c, i) {
            c *= factor;
    ...
    Color.prototype.opposite = function(onWhich) {
        if(!onWhich) onWhich = [true, true, true];
    ...
    Color.prototype.addColor = function(color, onWhich) {
        if(!onWhich) onWhich = [true, true, true, true];
        var rgb1 = [this._red, this._green, this._blue, this._alpha],
            rgb2 = [color._red, color._green, color._blue, color._alpha],
            rgbF = rgb1;
        for(var i = 0; i < 4; i++) if(onWhich[i]) rgbF[i] = Math.max(rgb1[i], rgb2[i]) - Math.min(rgb1[i], rgb2[i]);
        return new Color('rgba(' + rgbF[0] + ', ' + rgbF[1] + ', ' + rgbF[2] + ', ' + rgbF[3] + ')');
    };

    Color.prototype.rgb = function(r, g, b) {
        console.log(this._red, this._green, this._blue);
        r = Math.floor(this._red + r);
        r = Math.max(0, Math.min(r, 255));
        g = Math.floor(this._green + g);
        g = Math.max(0, Math.min(g, 255));
        b = Math.floor(this._blue + b);
